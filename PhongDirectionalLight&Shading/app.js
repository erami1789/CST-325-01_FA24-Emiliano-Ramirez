...
function updateAndRender() {
    ...
    if (appInput.left) {
        ...
        var newX = cosAngle * lightDirection.x - sinAngle * lightDirection.z;
        var newZ = sinAngle * lightDirection.x + cosAngle * lightDirection.z;
        lightDirection.x = newX;
        lightDirection.z = newZ;
    }
    if (appInput.right) {
        cosAngle = Math.cos(-rotationSpeed);
        sinAngle = Math.sin(-rotationSpeed);
    ...
    if (appInput.up) {
        cosAngle = Math.cos(rotationSpeed);
        sinAngle = Math.sin(rotationSpeed);
        var newY = cosAngle * lightDirection.y - sinAngle * lightDirection.z;
        var newZ = sinAngle * lightDirection.y + cosAngle * lightDirection.z;
        lightDirection.y = newY;
        lightDirection.z = newZ;
    }
    if (appInput.down) {
        cosAngle = Math.cos(-rotationSpeed);
